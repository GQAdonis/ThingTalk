...
function compileTriggerOrQuery(ast, paramAccess) {
    ...
    for (let filter of ast.filters) {
        ...
        const ifstmt = new JSIr.IfStatement(cond);
        irBuilder.add(ifstmt);
        irBuilder.pushBlock(ifstmt.iffalse);
        irBuilder.add(new JSIr.Return('false'));
        irBuilder.popBlock();
    }
    irBuilder.add(new JSIr.Return('true'));
...
function compileQuery(ast) {
    ...
    return Ir.Invocation.Query(ast.selector, ast.channel, inputFn, filterFn, outputFn);
}
...
function compileAction(ast) {
    ...
    let inputFn = compileInputParams(ast);
    return Ir.Invocation.Action(ast.selector, ast.channel, inputFn);
}
